...
# log shortcuts
# spanner.*[0-9]s$
alias tracem="stern linkbatchd -s 1s | grep -i --line-buffered -E '\[cleanup\]|cleanup.*failed|\[summary|csv\]|decr=|cleanupall|distri|decr=.*input=.*date=[0-9]{4}-[0-9]{2}-[0-9]{2}|failed.*|ccf.*\.go|accts=.*runid=.*|monthrecords.*[0-9]s$|notify=true|broadcast=|leader.active.*\(me|heartbeat:.*|[0-9]*m[0-9]*\.[0-9]*s$|dbg\]|details\ only|dstore|cur\.go|drift\.go|export'"
